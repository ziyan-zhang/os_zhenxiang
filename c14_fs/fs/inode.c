...
void inode_release(struct partition* part, uint32_t inode_no) {
   ...
   /* 1.a 先将前12个直接块存入all_blocks */
   while (block_idx < 12) {
      all_blocks[block_idx] = inode_to_del->i_sectors[block_idx];
      block_idx++;
   }

   /* 1.b 如果一级间接块表存在，将其128个间接块读到all_blocks[12~]，并释放一级间接块表占用的扇区 */
   ...
   硬盘上的数据不需要清零，可以直接覆盖*/
   void* io_buf = sys_malloc(1024); //一个扇区512字节，inode最多跨两个扇区
...
void print_inode(struct inode* inode) {
   printk("my printing inode:\n");
	printk("i_no: %d\n", inode->i_no);
	printk("i_open_cnts: %d\n", inode->i_open_cnts);
	printk("i_sectors[0]: %d\n", inode->i_sectors[0]);
	printk("i_sectors[1]: %d\n", inode->i_sectors[1]);
	printk("i_sectors[2]: %d\n", inode->i_sectors[2]);
	printk("i_size: %d\n", inode->i_size);
	// printk("inode_tag: %d\n", inode->inode_tag);
}