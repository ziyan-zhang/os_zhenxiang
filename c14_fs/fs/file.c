...
rollback:
    switch (rollback_step) {
        case 3: //case 3, case 2没有break，是一种累加的回滚
            /* 创建inode，创建文件表项 | 父目录中 同步 新inode的目录项 到硬盘。将file_table中相应位清空 */
            memset(&file_table[fd_idx], 0, sizeof(struct file));
        case 2:
            /* 创建inode | 创建文件表项，父目录项中 同步 新inode的目录项 到硬盘。将新的inode释放 */
            sys_free(new_file_inode);
        case 1:
            /* | 创建inode，创建新inode文件表项，同步新inode目录项的增加。
            新inode创建失败，之前位图中分配的inode_no也要恢复 */
...
int32_t file_open(uint32_t inode_no, uint8_t flag) {
    ...
    file_table[fd_idx].fd_pos = 0;  //每次打开文件，都要将fd_pos还原为0，即让文件内的指针指向开头
    file_table[fd_idx].fd_flag = flag;
...
/* 将buf中count个字节写入file，成功则返回写入的字节数，失败则返回-1 */
int32_t file_write(struct file* file, const void* buf, uint32_t count) {

    if ((file->fd_inode->i_size + count) > (BLOCK_SIZE * 140)) {
        printk("exceed max file_size 71680 byets, write file failed\n");
        return -1;
    }
    ...
    uint32_t* all_blocks = (uint32_t*)sys_malloc(BLOCK_SIZE + 48);  //用来记录文件的所有块地址
    if (all_blocks == NULL) {
    ...
    if (file->fd_inode->i_sectors[0] == 0) {
		printk("first time write, alloc a block\n");
    ...
    /* 存储count个字节后该文件将占用的块数 */
    uint32_t file_will_use_blocks = (file->fd_inode->i_size + count) / BLOCK_SIZE + 1;
    ASSERT(file_will_use_blocks <= 140);

    /* 通过此增量判断是否需要分配扇区，如增量为0，表示原扇区够用 */
    uint32_t add_blocks = file_will_use_blocks - file_has_used_blocks;
    printk("has_used_blocks: %d, add_blocks: %d, will blocks: %d\n", file_has_used_blocks, add_blocks, file_will_use_blocks);
    ...
    if (add_blocks == 0) {
      ...
      if (file_will_use_blocks <= 12) {
         ...
         printk("first time write assigned a sector, inode is:\n");
         // print_inode(file->fd_inode);

      } else {
         /* 未写入新数据之前已经占用了间接块，需要将间接块地址读进来 */
         ASSERT(file->fd_inode->i_sectors[12] != 0);
         indirect_block_table = file->fd_inode->i_sectors[12];
         ide_read(cur_part->my_disk, indirect_block_table, all_blocks+12, 1); // hd, lba, buf, sec_cnt
      }
    } else {
      ...
      if (file_will_use_blocks <= 12) {
         ...
         /* 先将有剩余空间的可继续用的扇区地址写入all_blocks */
          ...
          ASSERT(file->fd_inode->i_sectors[block_idx] != 0);	//todo: 这里，如果file->fd_inode->i_size是BLOCK_SIZE整数倍，那就断言失败了呀
          all_blocks[block_idx] = file->fd_inode->i_sectors[block_idx];

          /* 再将未来要使用的扇区分配好后写入all_blocks */
          ...
          while (block_idx < file_will_use_blocks) {
            ...
            /* 写文件时，不应该存在块未使用，但已分配扇区的情况，当文件删除时，就会把块地址清零 */
         ...
         /* 先将有剩余空间可用的扇区地址收集到all_blocks */
         block_idx = file_has_used_blocks - 1;  //指向旧数据所在最后一个扇区
         all_blocks[block_idx] = file->fd_inode->i_sectors[block_idx];
         ...
         block_idx = file_has_used_blocks;   //第一个未使用的块，即本文件已使用的直接块的下一块
         while (block_idx < file_will_use_blocks) {
            block_lba = block_bitmap_alloc(cur_part);
            if (block_lba == -1) {
               printk("file_write: clock_bitmap_alloc for situation 2 failed\n");
               return -1;
            }
         ...
         ide_write(cur_part->my_disk, indirect_block_table, all_blocks+12, 1);   //同步一级间接表块到硬盘
         //todo 思考下这里的sector[12]同步, 和前12个i_sector的同步的异同。前12个sector中的更改是怎样同步的？间接块是怎样同步的
...
int32_t file_read(struct file* file, void* buf, uint32_t count) {
   ...
   uint32_t read_blocks = block_read_start_idx - block_read_end_idx; //todo:无符号所以可以前减后？
   ASSERT(block_read_start_idx<139 && block_read_end_idx<139); //todo:不能等于139？
   ...
   while (bytes_read < size) {
	...
	memset(io_buf, 0, BLOCK_SIZE);
	ide_read(cur_part->my_disk, sec_lba, io_buf, 1);	//io_buf是死的，memcpy是活的
