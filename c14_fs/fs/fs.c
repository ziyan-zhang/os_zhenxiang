...
static void partition_format(struct partition* part) {
	...
	/* 找出数据量大的元信息，用其尺寸做存储缓冲区 */
	uint32_t buf_size = (sb.block_bitmap_sects >= sb.inode_bitmap_sects ? sb.block_bitmap_sects : sb.inode_bitmap_sects);

	buf_size = (buf_size >= sb.inode_table_sects ? buf_size : sb.inode_table_sects) * SECTOR_SIZE;
	uint8_t* buf = (uint8_t*)sys_malloc(buf_size);	//从堆中申请内存缓冲区；有内存管理系统清零后返回；须要自己维护(line127释放)
	...
	*/
	...
	uint32_t last_size = SECTOR_SIZE - (block_bitmap_last_byte % SECTOR_SIZE);	//位图所在最后一扇区中，不足一扇区的部分（字节对齐更少）

	/* 2.1 先将位图最后一字节到其所在扇区的结束全部置为1，即超出实际块数的部分直接置为己用 */
	...
	*/
	...
	i->i_sectors[0] = sb.data_start_lba;	//只用这一个直接索引快就好了
	...
	*/
	...
	/* 初始化当前目录"." */
	memcpy(p_de->filename, ".", 1);		//dist, src, size
	p_de->i_no = 0;
	p_de->f_type = FT_DIRECTORY;
	p_de++;

	/* 初始化当前目录父目录 ".." */
	memcpy(p_de->filename, "..", 2);	//一个"."一字节，两个".."两字节
	p_de->i_no = 0;
	p_de->f_type = FT_DIRECTORY;
	...
	sys_free(buf);	//堆内存要自己释放，见line65
}

void filesys_init() {
	uint8_t channel_no = 0, dev_no, part_idx = 0;

	/* sb_buf用来存储从硬盘上读取的超级块 */
	struct super_block* sb_buf = (struct super_block*)sys_malloc(SECTOR_SIZE);

	if (sb_buf == NULL) {
		PANIC("alloc memory failed!");
	}
	printk("searching filesysytem...\n");
	while(channel_no < channel_cnt) {	//先遍历channel
		dev_no = 0;
		while (dev_no < 2) {
			...
			while (part_idx < 12) {
				...
				/* partition未初始化的话，成员是0 */
				if (part->sec_cnt != 0) {
					memset(sb_buf, 0, SECTOR_SIZE);
					/* 读出分区的超级块，根据魔数是否正确来确定是否存在文件系统 */
