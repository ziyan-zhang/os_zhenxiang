...
bool sync_dir_entry(struct dir* parent_dir, struct dir_entry* p_de, void* io_buf) {
   ...
   // printk("syncronizing dir entry\nthis inode:\n");
   // print_inode(dir_inode);


   // printk("dir size is %d\n", dir_size);
   // printk("dir_entry size is %d\n", dir_entry_size);
...
bool delete_dir_entry(struct partition* part, struct dir* pdir, uint32_t inode_no, void* io_buf) {
   ...
   bool is_dir_first_block = false; //目录第一个块 //todo: what?

   /* 遍历所有块，寻找目录项 */
   block_idx = 0;
   while (block_idx < 140) {
      is_dir_first_block = false;
      if (all_blocks[block_idx] == 0) {   //是个空块，转向处理下一块
         block_idx++;
         continue;
      }
      ...
      /* 遍历所有的目录项，统计该扇区目录项数量及是否有待删除的目录项 */
      while (dir_entry_idx < dir_entries_per_sec) {
         if ((dir_e + dir_entry_idx)->f_type != FT_UNKNOWN) {
            if (!strcmp((dir_e+dir_entry_idx)->filename, ".")) {
               ...
               dir_entry_cnt++; //统计此扇区内目录项个数
               if ((dir_e+dir_entry_idx)->i_no == inode_no) {  //如果找到此i结点，就将其记录在dir_entry_found中
      ...
      if (dir_entry_cnt == 1 && !is_dir_first_block) {
         ...
         if (block_idx < 12) {
            ...
            /* 先判断一级间接索引表中间接块的数量，
            如果仅有这一个间接块，连同间接索引表所在的块一块回收 */
            uint32_t indirect_blocks = 0;       //含有要找的目录项的块在indirect blocks中的索引
            uint32_t indirect_blocks_idx = 12;  //含有要找的目录项的块在all_blocks中的索引
            while (indirect_blocks < 140) {
               if (all_blocks[indirect_blocks_idx] != 0) {  //间接块索引表中有效间接块的数量（除要找的项所在块之外，是否还有其他块valid）
                  indirect_blocks++;
               }
            }
            ASSERT(indirect_blocks >= 1); //包括当前间接块

            if (indirect_blocks > 1) { //只改当前块
               all_blocks[block_idx] = 0;
               ide_write(part->my_disk, dir_inode->i_sectors[12], all_blocks+12, 1);
            } else { //间接索引表中就当前这一个间接块，直接把索引表所在的块回收，然后擦除间接索引表块地址
               /* 回收间接索引表所在的块 */
      ...
      inode_sync(part, dir_inode, io_buf);   //应该这里的buf由主调函数提供，不用包含信息量，只是保证有足够内存防止做无用功

      return true;
   }
   /* 所有块中未找到则返回false，出现这种情况应该是search_file出错了 */
   return false;
}

/* 读取目录，成功返回一个目录项，失败返回NULL */
struct dir_entry* dir_read(struct dir* dir) {
   struct dir_entry* dir_e = (struct dir_entry*)dir->dir_buf;  //todo这个dir_buf怎么用的
   ...
   while (dir->dir_pos < dir_inode->i_size) {
      ...
      ide_read(cur_part->my_disk, all_blocks[block_idx], dir_e, 1);  // 先根据block_idx遍历块
      dir_entry_idx = 0;
      /* 再根据dir_entry_idx遍历每个块内的目录项 */
      while (dir_entry_idx < dir_entrys_per_sec) {
         if ((dir_e + dir_entry_idx)->f_type) {
            /* 判断是不是最新的目录项，避免返回已经返回过的目录项 */
            if (cur_dir_entry_pos < dir->dir_pos) {
               cur_dir_entry_pos += dir_entry_size;
               dir_entry_idx++;
               continue;
            }  //此循环：根据dir->dir_pos找到dir_entry_idx，直接除也未尝不可吧；
