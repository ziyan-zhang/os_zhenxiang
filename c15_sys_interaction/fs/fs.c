...
int32_t sys_close(int32_t fd) {
	int32_t ret = -1;	//返回值默认-1，即失败
	if (fd > 2) {
		uint32_t _fd = fd_local2global(fd);
		ret = file_close(&file_table[_fd]);
		running_thread()->fd_table[fd] = -1;	//使文件描述符位可用
	}
	return ret;
}

/* 将buf中count个字节写入文件描述符fd，
成功则返回写入的字节数，失败返回-1 */
...
int32_t sys_lseek(int32_t fd, int32_t offset, uint8_t whence) {
   ...
   switch(whence) {
      /* SEEK_SET 新的读写位置是相对于文件开头再增加offset个偏移量 */
      case SEEK_SET:
         new_pos = offset;
         break;

      /* SEEK_CUR 新的读写位置是相对与当前位置增加offset个位移量 */
      case SEEK_CUR:
...
int32_t sys_unlink(const char* pathname) {
   ...
   if (searched_record.file_type == FT_DIRECTORY) {   //扫描的是目录项，目录项类型可以是文件或目录
      printk("can't delete a directory with unlink(), use rmdir() isntead\n");
      dir_close(searched_record.parent_dir);
      return -1;
   }
...
int32_t sys_mkdir(const char* pathname) {
   ...
   if (inode_no != -1) {
      ...
      uint32_t pathname_depth = path_depth_cnt((char*)pathname);
      uint32_t path_searched_depth = path_depth_cnt(searched_record.searched_path);
      if (pathname_depth != path_searched_depth) { //并没有访问到全部路径，某个中间目录是不存在的
         printk("sys_mkdir:cannot access %s: Not a directory, subpath %s doesnot exist\n", pathname, searched_record.searched_path);
         rollback_step = 1;
         goto rollback;
      }
   }
   ...
   struct dir* parent_dir = searched_record.parent_dir;
   /*  目录名称后可能会有字符‘/’,所以最好用searched_record.searched_path，无'/'  */
   ...
   if (block_lba == -1) {
      printk("sys_mkdir: block_bitmap_alloc fort create directory failed\n");
      rollback_step = 2;
      goto rollback;
   }
   ...
   struct dir_entry* p_de = (struct dir_entry*)io_buf;   //用p_de扫描io_buf
   ...
   p_de->f_type = FT_DIRECTORY;
   ide_write(cur_part->my_disk, new_dir_inode.i_sectors[0], io_buf, 1); //两个目录项也就6*2 = 12 字节？
   ...
   memset(io_buf, 0, SECTOR_SIZE*2);   // 清空io_buf，提供给sync_dir_entry
...
/* 创建文件或目录需要相关的多个资源，若某步失败则会执行到下面的步骤 */
rollback:
...
struct dir* sys_opendir(const char* name) {
   ...
   /* 如果是根目录'/'，直接返回&root_idr */
   if (name[0] == '/' && (name[1] == 0 || name[0] == '.')) {
      return &root_dir; //root_dir是定义在dir.h中的全局变量
   }
...
/* 读取目录dir的一个目录项，成功后返回其目录项地址，到目录末尾时或出错时返回NULL */
...
int32_t sys_rmdir(const char* pathname) {
   ...
   if (inode_no == -1) {
      ...
      if (!dir_is_empty(dir)) {
         printk("dir %s is not empty, not allowed to be deleted\n", pathname);
      } else {
   ...
   dir_close(searched_record.parent_dir); //search_file要自己关闭parentdir
   return retval;
}
...
static uint32_t get_parent_dir_inode_nr(uint32_t child_inode_nr, void* io_buf) {
   ...
   /* 目录的目录项".."中包括父目录的inode编号，".."位于目录的第0块 */
...
char* sys_getcwd(char* buf, uint32_t size) {
   ...
   即子目录在前（左），父目录在右（右），
   先将full_path_reverse中的路径反置 */
   char* last_slash;
   while ((last_slash = strrchr(full_path_reverse, '/'))) {
      uint16_t len = strlen(buf);
      strcpy(buf+len, last_slash);
      /* 在full_path_reverse中添加结束字符，作为下一次执行strcpy中lash_slash的边界 */
      *last_slash = 0;
   }
   sys_free(io_buf);
   return buf;
