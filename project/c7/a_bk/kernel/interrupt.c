...
static void general_intr_handler(uint8_t vec_nr) {
   ...
   //如果为pagefault，将缺失的地址打印出来并且悬停
   if (vec_nr == 14) { 
         int page_fault_vaddr = 0;
         asm volatile ("movl %%cr2, %0": "=r" (page_fault_vaddr)); //cr2存放造成pagefault的虚拟地址
         put_str("\npage fault addr is: ");put_int(page_fault_vaddr);
   }
   put_str("!!!!!!!!!!!exception message end!!!!!!!!！!");
   //能进入中断处理程序就表示已经处于关中断的情况下，不会出现进程调度的情况，因此下面的死循环可以一直执行
   while (1);
}
